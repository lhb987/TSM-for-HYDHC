...
def parse_args():
    ...
    parser.add_argument(
        '--txt',
        type=str,
        default='/131_data/DHC/all_videofolder.txt'
    )
...
if __name__ == '__main__':
    ...
    with open(args.txt, 'r') as f:
        frame_dirs = [line.strip().split(' ')[0] for line in f.readlines()]
        full_path_dirs = [os.path.join(args.src_dir, frame_dir) for frame_dir in frame_dirs]
        for full_path_dir in full_path_dirs:
    ...
    if args.input_frames:
        ...
        # fullpath_list = glob.glob(args.src_dir + '/*' * 5 +'.jpg')
        # fullpath_list += glob.glob(args.src_dir + '/*' * 6 +'.jpg')
        # print('Total number of rgb frame folders found: ', len(fullpath_list))
        print('Total number of rgb frame folders found: ', len(full_path_dirs))
    else:
        print('For DHC, We should use frames as an input')
    ...
    # vid_list = list(map(
    #     lambda p: '/'.join(p.split('/')[4:]), fullpath_list
    # ))
    ...
    vid_list = list(map(
        lambda p: '/'.join(p.split('/')[4:-1]), full_path_dirs
    ))
    # import pdb;pdb.set_trace()
    ...
    pool.map(
        extract_frame,
        zip(full_path_dirs, vid_list, range(len(vid_list)),
            len(vid_list) * [args.flow_type],
            len(vid_list) * [args.task],
            len(vid_list) * [args.report_file]))
    pool.close()
